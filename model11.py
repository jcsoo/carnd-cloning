...
def main(args):
    ...
    samples = loader.load_data(args)
    train_samples, validation_samples = train_test_split(samples, test_size=0.2)

    print('train_samples:', len(train_samples), 'validation_samples:', len(validation_samples))
    
    tg = loader.training_generator(train_samples, batch_size=32, correction=0.25)
    vg = loader.validation_generator(validation_samples, batch_size=32)
